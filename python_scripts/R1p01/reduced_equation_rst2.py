...
problem.add_equation("div(Grad(p)) + lift(tau_p1) = div(Grad(K*T))", condition='nz==0')
...
# Initial conditions
f = h5py.File('snapshots_rst_s201.h5')
p.load_from_hdf5(f, 0, task='<Field 23017976173888>')
dt = f['scales/timestep'][0]/10

# Analysis
snapshots = solver.evaluator.add_file_handler('snapshots_rst2', sim_dt=0.01, max_writes=1, parallel='gather')
snapshots.add_tasks(solver.state)

slices = solver.evaluator.add_file_handler('slices_rst2', sim_dt=0.0005, max_writes=100, parallel='gather')
