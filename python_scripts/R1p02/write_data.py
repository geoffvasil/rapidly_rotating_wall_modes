...
f = h5py.File('snapshots_rst/snapshots_rst_s200.h5')
p = dist.Field(bases=(z_basis, disk_basis))
p.load_from_hdf5(f, 0, task='<Field 22301677885408>')
qz = dist.Field(bases=(z_basis, disk_basis.edge))
qz.load_from_hdf5(f, 0, task='<Field 22301677885504>')
...
cond_flux = d3.integ(dz(T), polar_coords).evaluate()
A = np.pi*0.2**2
print(cond_flux(z=0)['g']/A/112.95141083744463/1.02/0.02)
print(cond_flux(z=0)['g']/A/112.95141083744463/1.02)
