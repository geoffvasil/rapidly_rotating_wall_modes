...
tau_p = dist.Field(bases=disk_basis.edge)

K = dist.Field(bases=(z_basis))
hz = Lz/(2*np.pi)
K['g'] = hz*sum(np.sin(2*k*z/hz)/k for k in range(1,Nz//2))
K = d3.Grid(K)
...
problem.add_equation("qphi +  H(qz) = 0", condition='nz!=0')
problem.equations[-1]["valid_modes"][1::2] = False # cos only

problem.add_equation("div(Grad(p)) + lift(tau_p) = div(Grad(K*T))", condition='nz==0')
problem.add_equation("p(r=r0) = 0", condition='nz==0')
problem.add_equation("qphi = 0", condition='nz==0')
...
# Initial conditions
p.fill_random('g', seed=30, distribution='normal', scale=0.01) # Random noise
p.low_pass_filter(scales=0.5)

# Analysis
snapshots = solver.evaluator.add_file_handler('snapshots', sim_dt=0.01, max_writes=1, parallel='gather')
snapshots.add_tasks(solver.state)

slices = solver.evaluator.add_file_handler('slices', sim_dt=0.0005, max_writes=100, parallel='gather')
...
# CFL
CFL = d3.CFL(solver, initial_dt=max_timestep, cadence=1, safety=safety, threshold=0.05,
