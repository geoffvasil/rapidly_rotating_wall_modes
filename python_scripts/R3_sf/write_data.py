...
f = h5py.File('snapshots/snapshots_s24.h5')
p = dist.Field(bases=(z_basis, disk_basis))
p.load_from_hdf5(f, 0, task='<Field 22533415310464>')
qz = dist.Field(bases=(z_basis, disk_basis.edge))
qz.load_from_hdf5(f, 0, task='<Field 22533415313872>')
qphi = dist.Field(bases=(z_basis, disk_basis.edge))
qphi.load_from_hdf5(f, 0, task='<Field 22533415310560>')
...
divq = divq.evaluate()
divq.change_scales(4)
divq = divq['g']

curlq = dphi(qz) - dz(qphi)
curlq = curlq.evaluate()
curlq.change_scales(4)
curlq = curlq['g']
...
data = {'T_wall':T_wall, 'p_wall':p_wall, 'T_mid':T_mid, 'p_top':p_top, 'T_int': T_int, 'F_c':conv_flux, 'F_d':cond_flux, 'qz': qz['g'], 'qphi':qphi['g'], 'divq':divq, 'curlq':curlq, 'z':z, 'z_hres':z_hres, 'r': r, 'phi':phi}
