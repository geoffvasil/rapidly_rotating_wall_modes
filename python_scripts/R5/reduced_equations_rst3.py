...
# Initial conditions
f = h5py.File('snapshots_rst2/snapshots_rst2_s2.h5')
p.load_from_hdf5(f, 0, task='<Field 23098988402032>')
dt = f['scales/timestep'][0]/10

# Analysis
snapshots = solver.evaluator.add_file_handler('snapshots_rst3', sim_dt=0.0005, max_writes=1, parallel='gather')
snapshots.add_tasks(solver.state)

slices = solver.evaluator.add_file_handler('slices_rst3', sim_dt=0.00005, max_writes=100, parallel='gather')
slices.add_task(d3.integ(p, 'z')/Lz, name='<p>')
slices.add_task(T(z=0.05), name='T(z=0.05)')
